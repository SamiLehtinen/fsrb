...
.DESCRIPTION
    ...
    Rules:
      ...
      - Empty SEARCH block is allowed only for CREATE operations (empty SEARCH + non-empty REPLACE).
...
# NOTE: Patch header paths behave like the Rust implementation:
# they are used as provided and resolved relative to the process working directory.
# Sync .NET/OS CWD with PS current location so that all System.IO.* methods
# (File::Exists, StreamReader, FileInfo, File::Move, etc.) resolve relative
# paths against the same root that Resolve-Path and Test-Path use (i.e. CWD).
...
function Read-AllTextStrictUtf8([string] $Path) {
    # Match Rust's fs::read_to_string behavior more closely:
    # do NOT strip BOMs via BOM-detection; decode as strict UTF-8 and let any BOM become U+FEFF.
    $enc = New-StrictUtf8Encoding
    $reader = [System.IO.StreamReader]::new($Path, $enc, $false)
    try {
...
function Parse-PatchFile {
    ...
    function Split-RustLines([string] $Text) {
        # Emulate Rust's str::lines():
        # - split on '\n'
        # - strip a single trailing '\r' (CRLF)
        # - do NOT split on lone '\r'
        # - if the input ends with '\n', do NOT include a final empty line
        $parts = $Text.Split("`n", [System.StringSplitOptions]::None)

        $count = $parts.Length
        if ($count -gt 0 -and $Text.EndsWith("`n")) { $count-- }

        $out = [System.Collections.Generic.List[string]]::new($count)
    ...
    # Rust-parity line splitting for parsing: content.lines()
    $lines = Split-RustLines $Content
    ...
    function Resolve-TargetPath([string] $HeaderPath) {
        # Match Rust behavior: keep the header path as-is (no canonicalization).
        return $HeaderPath
    }
...
function Validate-AndApplyInMemory {
    ...
    $createFiles = New-Object System.Collections.Generic.SortedSet[string] ([System.StringComparer]::Ordinal)
    foreach ($b in $Blocks) {
    ...
    $unavailable = New-Object System.Collections.Generic.SortedSet[string] ([System.StringComparer]::Ordinal)
...
function Commit-ChangesAtomic {
    ...
    foreach ($p in $paths) {
        ...
        # Rust parity:
        # Create: if not existed_on_disk and content is not empty/whitespace
        # Delete: if existed_on_disk and content is empty/whitespace
        # Update: if existed_on_disk and content is not empty/whitespace
        $isEmptyContent = ([string]$st.ContentNorm).Trim().Length -eq 0
